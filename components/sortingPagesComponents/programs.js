...
import SortedInstructorsCard from './SortedInstructorsCard';
...
const carMentor = './assets/carMentor.png';
...
export default function SortPage({programData, schoolRows, instructorRows}){
    ...
    const mutateData = (sortedList) =>{
        var roster = [];
        for (const schoolName in sortedList){
            var mentorInfoArray = []
            for (const instructor of sortedList[schoolName]){
                mentorInfoArray.push(
                    {
                        ...
                        "multipleDays": 0,
                        "prevMentor": instructor['returner'],
                        "shirtSize": instructor['shirtSize'],
                        "university": instructor['university'],
                        "year": instructor['schoolYear'],
                        "teacherSchedule": demoSchedule,
                        "region": instructor['region'],
                        "schoolName": schoolName,
                        "schoolSchedule": demoSchedule,
                        "isLocked": 0,
                    }
                );
            }
            roster.push({"school":schoolName, "mentors":mentorInfoArray});
            console.log(roster);
        }
        setSchools(roster);
    };
    ...
    const resortYes = () => {
        ...
        // resortRoster()
        // .then((sortedList) => {
        //     console.log("PROMISE DONE=RESORTED!!!!",sortedList);
        //     // both have loaded!
        //     setIsLoading(!isLoading);
            
        //     mutateData(sortedList);
        // })
        const sortedList = Sort(instructorRows,schoolRows,'WebJam');
        // setIsLoading(!isLoading);
        mutateData(sortedList);
